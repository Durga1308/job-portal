...
const MyJobs = () => {
    ...
    useEffect(() => {
        setIsLoading(true)
        fetch(`http://localhost:3000/myJobs/durgasnd1308@gmail.com`)
        .then(res => res.json())
        .then(data => {
         setJobs(data);
         setIsLoading(false);
  ...
  <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded ">
    <div className="rounded-t mb-0 px-4 py-3 border-0">
      <div className="flex flex-wrap items-center">
        ...
        <Link to="/post-job"><button class="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">Post A new Jobs</button></Link>
        </div>
      </div>
    </div>
    ...
    <div className="block w-full overflow-x-auto">
      <table className="items-center bg-transparent w-full border-collapse ">
        ...
        {
            isLoadind ? (<div className='flex-center justify-center h-20'><p>loding....</p></div>) : (
                          <tbody>
            {
